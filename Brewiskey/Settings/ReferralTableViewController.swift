...
class ReferralTableViewController: UITableViewController {
    ...
    fileprivate func setupNibTableView() {
        let nibName = "HeaderCell"
        let cell = UINib(nibName: nibName, bundle: nil)
        tableView.register(cell, forCellReuseIdentifier: headerCellIdentifier)
        
        let paramNibName = "ParamTableViewCell"
        let paramCell = UINib(nibName: paramNibName, bundle: nil)
        tableView.register(paramCell, forCellReuseIdentifier: paramCellIdentifier)
    }
    
    @objc func buildFDLLink() {
        if ReferralTableViewController.DYNAMIC_LINK_DOMAIN == "YOUR_DYNAMIC_LINK_DOMAIN" {
        ...
        let components = DynamicLinkComponents(link: link, domain: ReferralTableViewController.DYNAMIC_LINK_DOMAIN)
...
//
extension ReferralTableViewController : UIGestureRecognizerDelegate {
...
//            return cell
        default:
            ...
            if row == 0 {
                ...
                cell.toggleButton.addTarget(self, action: #selector(ReferralTableViewController.toggleCollapse), for: .touchUpInside)
                return cell
            } else {
                let cell = tableView.dequeueReusableCell(withIdentifier: "param", for: indexPath) as! ParamTableViewCell
